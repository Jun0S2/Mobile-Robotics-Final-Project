...
r_react = 0.45;      %reaction radius
r_avoid = 1.5;        %avoidance radius
n_part = 6;         %number of partitions for reactive layer
n_sec = 72;                 % # of vfh sectors
a = r_avoid;                % vfh constants
b = 1;
vfh_threshold = 0.5;        % maximum "density" for a sector
window_size_threshold = 2;  % minimum sectors in window
react_threshold = 3;        % points
total_dist_traveled = 0;    % for scoring
goal_hdg = 0;
goal_hdg_global = pi/6;
last_dist = 0;
...
for i=1:Total_Steps
    ...
    goal_section_global = ceil( (mod(SD.TotalAngle(i),2*pi) - goal_hdg_global) / (2*pi/n_sec) );
    ...
    if(0)%any(p_react))%any(zone{1}))    %if any points within reaction radius
        fprintf("reacting!\n");
        ...
        for j = 1:length(zone)
            count(j) = sum(any(zone{j})); %count points in zone
        end
        
        %choose section closest to current heading
        for z = 1:size(count)
            if count(z) < react_threshold
                section_score(z) = abs(z-n_part/2);
            else section_score(z) = 0;
            end
            [~,ind] = max(section_score);
        end
        
        
        goal_hdg = SD.TotalAngle(i) - (ind-1)*pi/(n_part/2)
        fprintf("react set goal_hdg: %f\n",goal_hdg);
    else
        if(any(p_plan) && mod(i,2)==0 && any(zone{1}))
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %                    Do VFH Approach         %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            fprintf("planning...\n");
            ...
            for y = 1:length(window)
                ...
                if (length(curr_win)>window_size_threshold)
                    if (length(curr_win) <6)
                        window_center(y) = curr_win(ceil(length(curr_win)/2)); %go dead center of a small window.
                    else
                        window_center(y) = curr_win(ceil(length(curr_win)/3)); %shoot for the left of a large window
                    end
                    window_score(y) = abs(window_center(y) - n_sec/2 - goal_section_global);
                end
            end
            [~,window_ind] = max(window_score);
            goal_window = window_center(window_ind);
            if (goal_window ~= 1 || goal_window ~= n_sec)
    ...
    omega = 0.5*(Kp*err(i+1) + Ki*total_err(i+1) + Kd*d_err(i+1));
    ...
    %SetDriveWheelsSMART(S_Create, 0, 0, CliffLeft,CliffRight,CliffFrontLeft,CliffFrontRight,0,0,0);
    ...
    total_dist_traveled = total_dist_traveled + SD.Dist(i) %add incremental travel to total
    
    if(total_dist_traveled > last_dist + 10)
        last_dist = total_dist_traveled;
        goal_hdg_global = goal_hdg_global + randi([-3 3])*pi/6;
    end
    %----------------------------------------------------------
