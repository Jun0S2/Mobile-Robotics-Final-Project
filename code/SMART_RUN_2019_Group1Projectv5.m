...
b = 1;
vfh_threshold = 0.2;        % maximum "density" for a sector
window_size_threshold = 3;  % minimum sectors in window
...
for i=1:Total_Steps
    ...
    if(any(p_react))%any(zone{1}))    %if any points within reaction radius
        fprintf("reacting!\n");
        ...
        goal_hdg = SD.TotalAngle(i) + (ind-1)*pi/3;
        fprintf("react set goal_hdg: %f\n",goal_hdg);
    else
        if(any(p_plan) && mod(i,2)==0)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %                    Do VFH Approach         %
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            fprintf("planning...\n");
